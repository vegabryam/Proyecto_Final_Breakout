...
import proyecto_final_breakout.Vista.PanelJoker;
import proyecto_final_breakout.Vista.PanelMario;
...
public class HiloMario extends Thread {
    ...
    private PanelMario panelInicial;
    ...
    public HiloMario(PanelMario panelMario) {
        super("Hilo Barra");
        ...
        this.panelInicial = panelMario;
    }

    public PanelMario getPanelInicial() {
        return panelInicial;
    }

    public void setPanelInicial(PanelMario panelInicial) {
     ...
     public void rebotarPelota(){
        ...
        while(bandera==true){
            try {
                ...
                A:for(int i=0; i<this.panelInicial.getMapaM().getMapa().length;i++){
                    for(int j=0; j<this.panelInicial.getMapaM().getMapa()[0].length;j++){
                        if(this.panelInicial.getMapaM().getMapa()[i][j]>0){
                            int bloqueX=j*this.panelInicial.getMapaM().getBloqueancho()+25;
                            int bloqueY=i*this.panelInicial.getMapaM().getBloquealto()+29;
                            int bloqueancho=this.panelInicial.getMapaM().getBloqueancho();
                            int bloquealto=this.panelInicial.getMapaM().getBloquealto();
                            ...
                            if(rect1.intersects(rect)){
                                if(this.panelInicial.getMapaM().setBloqueValor(3, i,j)){
                                 this.panelInicial.getMoneda().start();                               
                                 this.panelInicial.getMoneda().loop(1);
