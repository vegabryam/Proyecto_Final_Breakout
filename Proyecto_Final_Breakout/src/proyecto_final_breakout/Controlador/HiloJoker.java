 ...
 * @author Asus
 */
public class HiloJoker extends Thread{
    
    private PanelJoker panelInicial;
    ...
    public HiloJoker(PanelJoker panelJoker) {
        super("Hilo Barra");
        ...
        while(bandera==true){
            try {
                ...
                if(new Rectangle(this.panelInicial.getBolaposX(),this.panelInicial.getBolaposY(),20,20).intersects(new Rectangle(this.panelInicial.getPosX(),this.panelInicial.getPosY(),100,10))){
                    ...
                    this.panelInicial.getRebotaJoker().start();
                    this.panelInicial.getRebotaJoker().loop(1);
                
                    
                }
                A:for(int i=0; i<this.panelInicial.getMapaG().getMapa().length;i++){
                    for(int j=0; j<this.panelInicial.getMapaG().getMapa()[0].length;j++){
                        if(this.panelInicial.getMapaG().getMapa()[i][j]>0){
                            int bloqueX=j*this.panelInicial.getMapaG().getBloqueancho()+100;
                            int bloqueY=i*this.panelInicial.getMapaG().getBloquealto()+20;
                            int bloqueancho=this.panelInicial.getMapaG().getBloqueancho();
                            int bloquealto=this.panelInicial.getMapaG().getBloquealto();
                            ...
                            if(rect1.intersects(rect)){
                                if(this.panelInicial.getMapaG().setBloqueValor(3, i,j)){
